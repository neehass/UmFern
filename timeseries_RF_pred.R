...
folder_MODELS <- "./RF_models"
folder_MODELS_KLASS <- "./RF_Klass_models"
folder_PRED <- "./RF_pred"
...
# load RF modles ------------------
# ---------------------------------------------------------
rf_model_WPI_RF <- readRDS(file.path(folder_MODELS, paste0("rf_model_WPI_RF3.rds")))
rf_model_F_RF <- readRDS(file.path(folder_MODELS, paste0("rf_model_F_RF3.rds")))
rf_model_P_RF <- readRDS(file.path(folder_MODELS, paste0("rf_model_P_RF3.rds")))

rf_model_WPI_RF_CLASS <- readRDS(file.path(folder_MODELS_KLASS, paste0("rf_model_WP_RF_class_NORM.rds")))
# ---------------------------------------------------------
# load landsat data ------------------
# ---------------------------------------------------------
files_raster <- list.files(data_crop_dir, recursive = TRUE, pattern=".tif$")

parts <- strsplit(files_raster, "_|\\.")
years <- sapply(parts, `[`, 5)
unique_years <- unique(years)
print(unique_years) # von 1985 bis 2025 >> 40Jahre
length(unique_years) # 26 Scenen 
...
# raster_years_norm <- lapply(raster_years, function(x){
#   band_means <- global(x, "mean", na.rm = TRUE)[,1]
#   norm <- x - band_means
#    return(norm) })
...
# ---------------------------------------------------------
folder_PRED_WPI <- file.path(folder_PRED, "WPI3")
...
png(file.path(output, "WPI3_interpolate_RF_pred.png"), height = 800, width = 800)
par(mfrow=c(7,4))
...
all_pred_WPI_Class <- func_pred_RF(raster_years_norm[20], rf_model_WPI_RF_CLASS, sampleloc_extent3, outpur_dir = folder_PRED_WPI_class)
