...
output <- "./R-scripts/2013ALL_Valid_elZrelli-outputs"
dir.create(output)

data_elZrelli_shp <- "./elZrelli2018_shp"
data_elZrelli <- "./elZrelli2018"

data_dir_valid <- "./landsat-1995_2025"
data_dir_valid_masekd <- "./landsat2013_HERBST-masked"
...
# plot(gabes_hafen)
# plot(gulf_shp, add = TRUE, col = "darkgrey", alpha = 0.2)
...
vaild_ALL_list <- func_timestep_sel(timestep, files_band, satNR_bands, unique_dates, data_dir_valid, time = "HERBST")
datesyear <- unique_dates[grep(paste(timestep, collapse = "|"), unique_dates)]
dates <- datesyear[substr(datesyear, 5, 6) %in% c("08","09", "10", "11")] # lasse "10" weg wegen wolken maske 

# mask cloud and extent 
validALL_masked_list <- func_mask_cloud_EXTENT(raster = vaild_ALL_list, data_dir = data_dir_valid, 
                                sampleloc_extent = sampleloc_extent, gabes_hafen = gabes_hafen)
idx0 <- sapply(validALL_masked_list, function(x) {
  class(x) == "numeric" # numeric und damit 0, dh zu hohen Wolken Anteil in HAfen region 
})

validALL_masked_list_clean <- validALL_masked_list[!idx0]
dates <- dates[!idx0]
...
plot(median_raster)
writeRaster(median_raster, file.path(data_dir_valid_masekd,"EXTENT_valid_sampleLoc_MEDIAN_HERBST2013.tif") , overwrite = TRUE)

# gulf_shp_pj <- project(gulf_shp, crs(median_raster))

# NDVI <- (median_raster$NIR - median_raster$Red) / (median_raster$Red + median_raster$NIR)
# plot(NDVI > 0.2)
# plot(gulf_shp_pj, add = TRUE, col = "darkgrey", alpha = 0.2)

# ---------------------------------------------------------
# crop sample locations ------------------
# ---------------------------------------------------------
# crop sample loc extent -----
sampleloc_points_pj <- project(sampleLoc_points, crs(vaild_SEP))

buffer <- buffer(sampleloc_points_pj, width = 10) # meter
valid_SEP_points <- mask(valid_SEP_Extent, buffer)
valid_SEP_points <- crop(valid_SEP_points, median_raster)
writeRaster(valid_SEP_points, file.path(data_dir_valid_masekd,"POINTS_valid_MEDIAN_HERBST2013.tif") , overwrite = TRUE)

# load 
valid_SEP_Extent <- rast(file.path(data_dir_valid_masekd,"EXTENT_valid_sampleLoc_MEDIAN_HERBST2013.tif"))
valid_SEP_points <- rast(file.path(data_dir_valid_masekd,"POINTS_valid_MEDIAN_HERBST2013.tif"))
...
valid_SEP_Extent_masked <- mask(valid_SEP_Extent, gulf_shp_pj,  inverse = TRUE)
names(valid_SEP_Extent_masked) <-names(valid_SEP_Extent)
...
values_long_INDEX <- valid_SEP_points_VALUES %>%
  select(Longitude, TMS_tassan, TMS_laili, log_Chl) %>%
  pivot_longer(cols = c(TMS_tassan, TMS_laili, log_Chl),
               names_to = "Index",
               values_to = "Value")
...
label_text <- paste0(
  ...
  " | R² = ", round(R2_CHLA,2), "</span>"
)

p_WPI_INDEX <- p_WPI_INDEX  +
  annotate("richtext",     x = max(df_corr$Longitude, na.rm = TRUE)-0.12,
         y = max(values_long_INDEX$Value, na.rm = TRUE)-2,
           ...
           size = 3)
p_WPI_INDEX
ggsave(file.path(output, "WPi_INDEX.png"), p_WPI_INDEX, height = 6, width = 6, scale = 1.2)
...
rf_df_sample$R_Blue <- rf_df_sample[,"Red"] / rf_df_sample[,"Blue"]
rf_df_sample$TSM_laili <- func_TMS_Laili(rf_df_sample[,"Blue"], rf_df_sample[,"Red"])
rf_df_sample$logChla <- func_log_Chl(rf_df_sample[,"Red"], rf_df_sample[,"Green"])
...
brks <- c( 0,1,2,3,4, 6, 10, 15, 30)#seq(0.3, 20,  by = 4)
RF_WPI <- func_RF_ranger(WPI_interpolate, RSdata_valid, model_name = "WPI_RF3", 
...
WPI_interpolate$WPI_class <- classify(WPI_interpolate$var1.pred, rcl)
WPI_interpolate$WPI_class <- as.factor(WPI_interpolate$WPI_class)
levels(WPI_interpolate$WPI_class) <- data.frame(ID = 1:5,class = c("Not affected", "Slightly affected","Moderately affected","Strongly affected","Seriously affected"))
